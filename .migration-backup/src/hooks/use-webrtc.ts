...
export function useWebRTC(sessionId: string, isInitiator: boolean, deviceName?: string) {
  ...
  const setupDataChannel = useCallback(
    (dc: RTCDataChannel) => {
      ...
      dc.onopen = () => {
        console.log("[QB] DataChannel OPEN");
      ...
      dc.onclose = () => {
        console.log("[QB] DataChannel CLOSED");
        stopQualityPoll();
      ...
      dc.onerror = (err) => { console.warn("[QB] DataChannel error", err); };
  ...
  const scheduleReconnect = useCallback(() => {
    if (!peerPresentRef.current) {
      console.log("[QB] scheduleReconnect: no peer present, setting waiting");
      setStatus("waiting");
      return;
    }
    if (reconnectAttemptRef.current >= MAX_RECONNECT_ATTEMPTS) {
      console.log("[QB] scheduleReconnect: max attempts reached, setting disconnected");
  ...
  const createPeerConnection = useCallback(() => {
    ...
    if (pcRef.current) {
      console.log("[QB] createPeerConnection: closing previous PC");
      try {
        pcRef.current.close();
      } catch {}
    }
    console.log("[QB] createPeerConnection: creating new RTCPeerConnection");
    ...
    pc.onconnectionstatechange = () => {
      ...
      console.log(`[QB] PC connectionState: ${st}`);
      if (st === "connected") {
        if (connectTimerRef.current) {
          clearTimeout(connectTimerRef.current);
          connectTimerRef.current = null;
    ...
    pc.oniceconnectionstatechange = () => {
      if (pc !== pcRef.current) return;
      console.log(`[QB] ICE connectionState: ${pc.iceConnectionState}`);
  ...
  const startOffer = useCallback(async () => {
    console.log("[QB] startOffer: creating offer");
    ...
    void tryComputeSas();
    console.log("[QB] startOffer: sending offer via signaling");
  ...
  useEffect(() => {
    ...
    console.log(`[QB] Signaling effect: sessionId=${sessionId}, role=${role}`);
    let aborted = false;
    ...
    if (stale) {
      console.log("[QB] Signaling: evicting stale lobby channel");
    ...
    const startHelloRetries = () => {
      ...
      const tick = () => {
        ...
        if (attempts >= 30) {
          console.log("[QB] hello retries exhausted (30)");
          return;
        }
        attempts += 1;
        if (attempts === 1 || attempts % 5 === 0) console.log(`[QB] hello attempt ${attempts}/30`);
        sendSignal({ type: "hello" });
    ...
    const presenceSyncHandler = () => {
      ...
      console.log(`[QB] presence sync: keys=${Object.keys(rawState).join(",") || "(empty)"}`);
      ...
      if (hasPeer && !wasPeerPresent) {
        console.log(`[QB] peer appeared (${isInitiator ? "guest" : "host"} detected)`);
        stopHelloRetries();
        if (isInitiator && !pcRef.current) {
          console.log("[QB] host: peer appeared, starting offer");
          startOffer();
        } else if (!isInitiator) {
          console.log("[QB] guest: peer appeared, sending hello");
          sendSignal({ type: "hello" });
        }
      }
      // Peer just left while we were active - update status
      if (!hasPeer && pcRef.current) {
        console.log("[QB] peer left, tearing down");
        teardownPeer();
        setQuality("unknown");
        setStatus("waiting");
    ...
    const signalHandler = async ({ payload }: { payload: unknown }) => {
      ...
      if (msg.type === "offer" && msg.sdp) {
        ...
        if (aborted) return;
        void tryComputeSas();
        console.log("[QB] sending ANSWER");
        sendSignal({ type: "answer", sdp: answer });
      } else if (msg.type === "answer" && msg.sdp) {
        console.log("[QB] received ANSWER");
        const pc = pcRef.current;
        ...
        console.log(`[QB] received HELLO from guest, existingPC=${!!existing}, healthy=${healthy}`);
        if (healthy) return;
        startOffer();
      }
    };
    ...
    const doSubscribe = (ch: typeof channel) => {
      ch.subscribe(async (s, err) => {
        console.log(`[QB] channel subscribe status: ${s}${err ? ` (${err.message ?? err})` : ""}`);
        if (s === "SUBSCRIBED") {
          subscribeRetries = 0;
          if (aborted) return;
          console.log(`[QB] tracking presence as ${role}`);
          await ch.track({
          ...
          console.log(`[QB] presence tracked successfully as ${role}`);
          if (aborted) return;
          if (!isInitiator) {
            console.log("[QB] guest: scheduling hello retries in 300ms");
            setTimeout(() => {
          ...
          console.log(`[QB] channel subscribe retry ${subscribeRetries}/${MAX_SUBSCRIBE_RETRIES} in ${Math.round(delay)}ms`);
  ...
  const manualReconnect = useCallback(() => {
    console.log(`[QB] manualReconnect called, peerPresent=${peerPresentRef.current}`);
    ...
    teardownPeer();
    if (!peerPresentRef.current) {
      console.log("[QB] manualReconnect: no peer, setting waiting");
      setStatus("waiting");
    ...
    setStatus("connecting");
    if (isInitiatorRef.current) {
      console.log("[QB] manualReconnect: host starting offer");
      void startOfferRef.current?.();
    } else {
      console.log("[QB] manualReconnect: guest sending hello");
      sendSignal({ type: "hello" });
    }
