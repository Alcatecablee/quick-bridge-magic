...
export function Session({ sessionId, isInitiator }: Props) {
  ...
  const shareQuickBridge = useCallback(() => {
    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      navigator.share({ title: "QuickBridge", text, url }).catch(() => {
  ...
  useEffect(() => {
    ...
    if (newSuccessCount > 0) { playSendSound(); maybeNudge(); }
  ...
  const handleFiles = useCallback(
    async (files: FileList | File[]) => {
      ...
      if (zipMode && filesToSend.length > 1) {
        ...
        try {
          const zipped = await zipFiles(filesToSend);
        ...
        {connected && (
          <div className="basis-full mt-1 divide-y divide-border/40 overflow-hidden rounded-md border border-border/40 bg-background/30">
            ...
            <div className="flex items-start justify-between gap-3 px-3 py-2">
              <div className="flex min-w-0 flex-col gap-0.5">
                <div className="flex items-center gap-2 text-[11px] font-medium text-foreground/90">
                  <Archive className="h-3.5 w-3.5 shrink-0 text-primary" />
                  <Label htmlFor="zip-mode" className="cursor-pointer text-[11px] font-medium">
                    Zip multi-file transfers
                  </Label>
                </div>
                <p className="pl-[22px] text-[10.5px] leading-snug text-muted-foreground/80">
                  Bundle multiple files into one zip before sending. One file prompt on the other end instead of many.
                </p>
              </div>
              <Switch
                id="zip-mode"
