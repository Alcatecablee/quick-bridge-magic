...
- **H1** is always `font-black` at `text-[46px] sm:text-[64px] md:text-[84px]` with `tracking-tight` - never `font-semibold`, never `leading-[1.1]`.
- **Highlight span** is always `text-muted-foreground` (subdued grey) - never `text-primary`.
- **Lead paragraph** is always `text-[16px] sm:text-[18px]` at `max-w-3xl`.

**Card / sub-section headings:** `text-[15px] font-semibold text-foreground` (never 14.5, never 16).

**Section h2:** `text-balance text-3xl font-bold tracking-tight text-foreground sm:text-4xl md:text-5xl`. Punchy single-statement h2s (e.g. "Open. Scan. Send.") use `font-black` instead of `font-bold`.
...
- Phase 2.5 (Presence capabilities broadcast) - done:
  - `src/lib/capabilities.ts` (new): detects local browser capabilities
    (clipboard, camera, microphone, location, storage/FSA). Validates/parses
    capability arrays from untrusted presence payloads.
  - `use-presence.ts`: broadcasts `capabilities: Capability[]` in every
    `trackSelf()` call. Extracts peer caps from join/sync/leave events.
    Exposes `peerCapabilities: ReadonlyMap<string, Capability[]>`.
  - `DevicesPanel.tsx`: detects local caps once on mount, passes to hook,
    persists capability snapshots to IDB on peer join, renders live capability
    hint (Camera, Clipboard, Disk write, etc.) below each online device row.
- Phase 8 (Trusted Environments - "Pair once. Forget forever.") - done:
  - ECDSA P-256 node identity; public key in IDB, private key non-exportable.
  - Mutual challenge/response handshake on reconnect; key-reset detection.
  - Trust prompt with double-tap guard and spinner.
  - Per-node presence channels (hashed IDs) with backoff and recovery.
  - One-click connect (fixed critical UUID-vs-session-ID validator bug).
  - All IDB writes settle on `tx.oncomplete` to survive quota rollback.
  - Trusted devices list: rename, remove, last-seen.
  - Known gaps: no one-click connect timeout/feedback; `tsc --noEmit`
    exits non-zero due to pre-existing `zip.ts` type error; `fromNodeId`
    received on trusted-connect but not surfaced in UI.
