...
import { ArrowLeft, KeyRound, Loader2 } from "@/components/quickbridge/icons";
...
export const Route = createFileRoute("/join")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "twitter:image:alt", content: "QuickBridge join with PIN: enter the 6-digit code shown on the other device" },
    ],
...
function JoinPage() {
  ...
  // Drives the sr-only aria-live region so screen readers hear status changes
  // (lookup start, success, and failure) without relying on toast alone.
  const [lookupAnnouncement, setLookupAnnouncement] = useState<string>("");
  const inputRef = useRef<HTMLInputElement>(null);
  // Track whether the component is still mounted so callbacks that fire after
  // a navigate-away (timeout, match broadcast) don't call setState or navigate
  // on a dead component. Also used to cancel in-flight channel/timer on unmount.
  const mountedRef = useRef(true);
  const pendingLookupCleanupRef = useRef<(() => void) | null>(null);

  useEffect(() => {
    mountedRef.current = true;
    return () => {
      mountedRef.current = false;
      pendingLookupCleanupRef.current?.();
      pendingLookupCleanupRef.current = null;
    };
  }, []);
  ...
  const submit = async () => {
    ...
    setBusy(true);
    setLookupAnnouncement("Looking for device...");
    ...
    const timer = window.setTimeout(() => {
      ...
      setBusy(false);
      setLookupAnnouncement("No device found for that PIN. Make sure the other device shows this PIN.");
      toast.error("No device found for that PIN", {
        description: "Make sure the other device is showing this PIN on its home screen.",
      });
    }, LOOKUP_TIMEOUT_MS);

    // Register so the unmount effect can cancel this lookup if the user
    ...
    channel.on("broadcast", { event: "match" }, ({ payload }) => {
      ...
      cleanup();
      if (!mountedRef.current) return;
      setLookupAnnouncement("Device found. Connecting...");
      navigate({ to: "/s/$id", params: { id: sid } }).catch(() => {
        if (!mountedRef.current) return;
        setBusy(false);
        setLookupAnnouncement("Could not open the session. Try again.");
        toast.error("Could not open the session", {
          description: "Navigation failed. Try again or reload the page.",
        });
      });
    });
    ...
    channel.subscribe((s) => {
      if (s === "SUBSCRIBED") {
        void channel.send({ type: "broadcast", event: "lookup", payload: { ts: Date.now() } }).catch(() => {});
      } else if (s === "CHANNEL_ERROR" || s === "TIMED_OUT") {
        // Supabase could not establish the channel (network error, service
        // degradation). Surface this immediately rather than letting the
        // 6-second timeout expire with a misleading "no device found" message.
        if (resolved) return;
        resolved = true;
        pendingLookupCleanupRef.current = null;
        window.clearTimeout(timer);
        ...
        if (!mountedRef.current) return;
        setBusy(false);
        setLookupAnnouncement("Could not reach the signaling service. Check your connection and try again.");
        toast.error("Could not reach the signaling service", {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      {/* Screen-reader live region: announces lookup state changes to assistive
          technology without relying on toasts, which vary in AT support. */}
      <div
        role="status"
        aria-live="polite"
        aria-atomic="true"
