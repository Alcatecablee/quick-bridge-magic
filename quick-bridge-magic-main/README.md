...
- Phase 8 - Trusted Environments ("Pair once. Forget forever."): **done**
  - ECDSA P-256 node identity generated per browser; public key stored
    in IndexedDB, private key non-exportable in the Web Crypto key store.
  - Trust prompt after first successful transfer; double-tap safe with
    spinner guard and in-flight ref preventing concurrent IDB writes.
  - Mutual challenge/response on reconnect: both sides sign each other's
    nonce; key-reset detection is distinct from impersonation rejection.
  - Per-node Supabase presence channels (hashed channel IDs for privacy)
    with exponential backoff and visibility/network recovery.
  - One-click connect: fixed silent failure caused by `crypto.randomUUID()`
    producing a 36-char hyphenated string that the protocol validator
    rejects. Session IDs now always use `generateSessionId()`.
  - All IDB writes resolve on `tx.oncomplete` (not `req.onsuccess`) to
    prevent false-success on storage-quota rollback.
  - Trusted devices list with rename, remove, and last-seen display.
  - Key files: `src/lib/node-identity.ts`, `src/lib/trusted-nodes-db.ts`,
    `src/lib/protocol.ts`, `src/hooks/use-presence.ts`,
    `src/components/quickbridge/DevicesPanel.tsx`,
    `src/components/quickbridge/TrustPrompt.tsx`.
